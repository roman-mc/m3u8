...
`
	...
	assert.Empty(t, dri.ClientAttributes)

	assertToString(t, line, dri)
}

func TestDateRangeItem_Parse_2(t *testing.T) {
	line := `#EXT-X-DATERANGE:ID="splice-6FFFFFF0",CUE="/DBCAAAAAAAAAP/wBQb+HcGWdQAsAipDVUVJAAAAA3//AACg5KYOFlBDS19QQ0tfVk9EXzgwMDAwMDc4MjMmAQGv3v0x",
START-DATE="2014-03-05T11:15:00Z"
`
	dri := NewDateRangeItem(line)
	...
	assert.Equal(t, "splice-6FFFFFF0", dri.ID)
	assert.Equal(t, "2014-03-05T11:15:00Z", dri.StartDate)
	assert.Equal(t, pointer.ToString("/DBCAAAAAAAAAP/wBQb+HcGWdQAsAipDVUVJAAAAA3//AACg5KYOFlBDS19QQ0tfVk9EXzgwMDAwMDc4MjMmAQGv3v0x"), dri.Cue)
